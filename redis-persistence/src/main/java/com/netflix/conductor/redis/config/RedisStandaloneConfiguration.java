...
public class RedisStandaloneConfiguration extends JedisCommandsConfigurer {
	...
	protected JedisCommands createJedisCommands(RedisProperties properties, ConductorProperties conductorProperties,
		...
		return new JedisStandalone(
				new JedisPool(config, host.getHostName(), host.getPort(), 1000, properties.getPassword()));

	}
	/*
	 * return new JedisStandalone(new JedisPool(config, host.getHostName(),
	 * host.getPort()));
	 */
	/*
	 * JedisPool pool = new JedisPool(config, host.getHostName(), host.getPort(),
	 * 1000, properties.getPassword());
	 * 
	 * try { Jedis jedis = pool.getResource(); System.out.
	 * println("RedisStandaloneConfiguration.createJedisCommands() connected"); }
	 * catch (JedisConnectionException e) { System.out.
	 * println("RedisStandaloneConfiguration.createJedisCommands() not connected");
	 * }
	 */
