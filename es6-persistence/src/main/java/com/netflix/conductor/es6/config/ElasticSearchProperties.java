...
public class ElasticSearchProperties {
	...
	/**
	 * The username for the elasticsearch password
	 */
	...
	/**
	 * The time in seconds after which the async buffers will be flushed (if no
	 ...
	 * be used to disable the use of specific document types with an override. This
	 * property is currently used in ES6 module.
	 * <p>
	 * <em>Note that this property will only take effect if
	 * {@link ElasticSearchProperties#isAutoIndexManagementEnabled} is set to false
	 * and index management is handled outside of this module.</em>
	 */
	...
	public List<URL> toURLs() {
		...
		return Arrays.stream(hosts)
				.map(host -> (host.startsWith("http://") || host.startsWith("https://") || host.startsWith("tcp://"))
						? toURL(host)
						: toURL("tcp://" + host))
